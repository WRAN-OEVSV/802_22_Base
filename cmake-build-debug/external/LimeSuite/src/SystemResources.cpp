...
std::string lime::locateImageResource(const std::string &name)
{
    ...
    {
        const std::string fullPath(searchPath + "/22.09/" + name);
...
{
    const std::string destDir(lime::getAppDataDirectory() + "/images/22.09");
    const std::string destFile(destDir + "/" + name);
    const std::string sourceUrl("https://downloads.myriadrf.org/project/limesuite/22.09/" + name);
